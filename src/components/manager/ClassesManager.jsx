...
import { classes as initialClasses, teachers, students } from '../../data/dummyData';
...
export default function ClassesManager() {
  ...
  const getStudentCount = (cls) => students.filter((s) => s.classId === cls.id).length;

  const openAdd = () => { setForm(emptyForm); setModal('add'); };
  const openEdit = (cls) => { setForm({ ...cls, teacherId: cls.teacherId?.toString() || '' }); setEditId(cls.id); setModal('edit'); };
  ...
  const handleSave = () => {
    ...
    if (modal === 'add') {
      const newCls = { ...form, id: form.name, studentIds: [], teacherId: form.teacherId ? parseInt(form.teacherId) : null, capacity: parseInt(form.capacity) };
      setClasses((prev) => [...prev, newCls]);
      showToast(`✅ تم إنشاء الفصل: ${form.name}`);
    } else {
      setClasses((prev) => prev.map((c) => c.id === editId ? { ...c, ...form, teacherId: form.teacherId ? parseInt(form.teacherId) : null, capacity: parseInt(form.capacity) } : c));
  ...
  return (
    <div className="flex flex-col gap-5 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        {classes.map((cls) => {
          const teacherObj = getTeacher(cls.teacherId);
          const stuCount = getStudentCount(cls);
          ...
          return (
            <div key={cls.id} className="bg-white rounded-3xl border border-gray-100 shadow-sm p-5 flex flex-col gap-4 hover:shadow-md transition-shadow">
              ...
              <div className="flex items-start justify-between">
                <div>
                  <span className={`text-xs font-bold px-2.5 py-1 rounded-xl ${colorBg[cls.color] || colorBg.blue}`}>{cls.gradeLevel}</span>
              ...
              <div className="flex items-center gap-2 bg-gray-50 rounded-2xl px-3 py-2">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-sm font-bold text-gray-700 truncate">{teacherObj?.name || 'غير معين'}</p>
                </div>
              </div>
      ...
      {/* Add/Edit Modal */}
      <FormModal isOpen={!!modal} title={modal === 'add' ? 'إضافة فصل جديد' : 'تعديل الفصل'} icon="🏫" onClose={() => setModal(null)} onSave={handleSave} saveLabel={modal === 'add' ? 'إضافة' : 'حفظ'}>
        <div className="grid grid-cols-2 gap-3">
          <Field label="اسم الفصل">
            <input name="name" value={form.name} onChange={handleChange} placeholder="مثال: KG1-C" className={inputCls} style={{ fontFamily: 'Cairo, sans-serif' }} />
          </Field>
          <Field label="المرحلة">
